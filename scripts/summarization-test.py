...
class Summarizer(pl.LightningModule):
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--resume_ckpt", type=str, help="Path of a checkpoint to resume from") # >> path where your check point is >> summariztion/test/version_0/checkpoints 
...
def main(args):
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath =os.path.join(args.save_dir, args.save_prefix, "checkpoints"), ## << dirpath for checkpoints 
        save_top_k=5,
        verbose=True,
    ...
    trainer = pl.Trainer(gpus=args.gpus, distributed_backend='ddp' if torch.cuda.is_available() else None,
                         ...
                         #resume_from_checkpoint=args.resume_ckpt, <<<<<< add path summarization/test/version_0/checkpoints
                         )
    if not args.test:
        trainer.fit(model)
    trainer.test(model)
